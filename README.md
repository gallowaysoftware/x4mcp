...
| `estimate_ship_cost`  | What a ship costs **fitted**, not the bare hull. Reads the hull's real slot list (L/M weapons, turrets, shields, engines) from the install and prices each — equipment is usually most of the bill, so a hull quote understates a warship by ~3x. Returns a min/avg/max band (component prices float with the economy), a total at **live** prices from discovered stations, and an explicit `markup` for wharf pricing the game data does not expose. |
| `list_plans`          | The player's OWN construction plans — every layout saved in the build menu, plus any Exported single-plan files. |
| `analyze_plan`        | Audit one of those plans for ratios: per-ware production vs internal consumption, which intermediates run a **deficit** (and the fractional module count that would close it), mined inputs to supply, sellable surplus, workforce jobs vs housing, storage. The inverse of `plan_production`/`plan_complex` — those design a station, this checks one you already drew. `sunlight`/`sector` scales the solar side, which can flip an energy deficit into a surplus. |
