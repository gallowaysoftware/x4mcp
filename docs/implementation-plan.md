...
**Status: implemented on `feat/x4cue-phase4-s6`, awaiting review. One gate FAILS — peak RSS. Write-up: `docs/s6-notes.md`.**
- [x] Capture: logbook (raw `{page,id}` + resolved text, `\033#…#` stripped) · stats · mission offers + war `group` attrs · licences + boosters · player inventory · Kha'ak/Xenon attrs-only (`knownto` kept — threat surfaces filter to `knownto=player`) · probe-informed fields (A/B/C outcomes)
- [x] Synthetic fixture per new section; goldens blessed (14_hull_states, 15_element_namesakes added; 01/07/08/09 extended)
- [~] Gate — real-save verification checklist (`docs/s6-notes.md` §4): logbook 17,004 · stats 103 · missions 30 offers / 33 missions · licences 182 · inventory 101 · threat 786 (reconciled exactly against a fresh scan) · **ParseMS 11,893 ms PASS (−1.07% vs the pre-S6 parse)** · **peak RSS 30.33 MB FAIL (+11.16 MB, the logbook owns 8.79 of it)** · in-game spot check NOT done · hitch check #2 NOT run. Original wording: logbook ≥ 6,503 matching fresh scanner run · stats/khaak/xenon counts matching a fresh scanner run **on the same save** (the frozen 103/601/5,345 were one save from 2026-08-10; the corpus already reads 105 stats, because these grow with play — compare against a re-run, never against the number written here) · missions/licences/inventory non-empty + spot-checked in game · ParseMS ≤ +10% of S2 baseline (3-run median) · RSS ≤ +5 MB · **hitch check #2**
- [x] Gate — **per-section marginal cost** (`internal/x4save/sections.go` + `BenchmarkParseSectionCost`; results in `docs/s6-notes.md` §4.3 and `docs/parse-baseline.md` §3.1): parse once with each new section individually disabled, and record what that section costs on its own. The aggregate +10%/+5 MB gate says a regression happened; it does not say which of five sections owns it, and a red that starts a bisect is a red that gets waived. (Owed to the fs25mcp session, which hit the same gate-shape problem from the other side.) **This is not free**: the parser dispatches sections from one element-name switch with no options struct, so it needs a test-only section mask (~20 lines) consulted at the new `case` arms. Budget it inside S6 rather than discovering it at the gate.
