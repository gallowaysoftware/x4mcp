...
|---|---|---|---|
| D1 | Watcher: fsnotify vs stat-poll | **2 s stat-poll with settle detection** (size+mtime stable across 2 ticks) | Both papers independently rejected fsnotify: X4 rewrites saves in place over seconds (a quiescence check is needed regardless), the house was already burned by fsnotify missing atomic renames (hum mounts a config *dir* for this reason), poll is dependency-free, identical on Proton/GOG/Windows paths, and ≤2 s detection is invisible against a 5–16 s parse and a 60 s freshness budget. PRD F2's "fsnotify" is treated as intent ("watch the save dir"), not implementation. **Amended by D15 (2026-08-12): the poll is kept as the correctness floor, with fsnotify added as an accelerator on top of it.** The reasoning below stands except where D15 says otherwise; it is left in place because the part that was wrong is worth being able to find. |
...
| D15 | Watcher, revisited: fsnotify as accelerator, poll as guarantee (amends D1) | **Hybrid.** The 2 s stat-poll stays and remains the thing correctness rests on. `github.com/fsnotify/fsnotify` watches the save DIRECTORIES; any event in one (create/write/rename/chmod — which one is never inspected) POKES the checker to run its ordinary stat+settle pass immediately. The poll relaxes to 10 s while events are arriving and tightens to 2 s when the watcher fails, goes quiet, or a save is mid-settle. fsnotify failing to initialise (inotify watch limits, an unsupported filesystem, permissions) logs one WARN, appears in the health payload, and changes nothing else: the process never fails to start over it | Three things, in order of how much they should have mattered. (1) **D1's rejection was partly inherited.** "The house was already burned by fsnotify missing atomic renames" is a real incident — hum mounts a config *directory* for exactly that reason — but it is evidence about a config file replaced by rename, not about how X4 writes a 100 MB savegame, and it was carried across as though it were. That is an argument from a scar, and it was doing more work in D1 than the X4-specific reasoning was. (2) **The poll's cost is negligible and was never the issue**: a stat over a handful of directories every few seconds does not register against a game using the whole machine, so "poll is cheap" is an argument for keeping the poll, never an argument against also being told. (3) **The accelerator buys ~4–6 s** of detection latency on the common path — an event arrives while X4 is still writing, so the settle pass starts at once and the confirming tick follows 2 s later, instead of the change waiting up to a full poll interval to be noticed at all. Neither half is trusted alone: X4 takes 20–60 s to write a late-game save and under Proton that is Wine translating Windows file ops, so no event sequence is knowable in advance or stable across game versions — quiescence is the only signal that means "finished", and the settle gate stays exactly as D1 specified. Conversely fsnotify can silently stop delivering (watch limits, a directory replaced wholesale), which is what the poll is for. **Instrumented so the choice is checkable**: `/api/state`'s watch section counts events received, pokes, and every detection attributed to whichever half saw it FIRST — `by_notify`, `by_poll`, and `missed_by_notify` (a poll detection while the watcher was up and watching). If fsnotify turns out to miss saves in practice, that counter is how we find out rather than the player. |

**Dependency delta (complete):** Go — `modernc.org/sqlite` (new), `github.com/fsnotify/fsnotify` (new, D15), `golang.org/x/sys` (indirect→direct), `github.com/google/jsonschema-go` (indirect→direct; one schema source for MCP face *and* agent tool defs), `github.com/gzuidhof/tygo` (tool directive only, never linked). npm runtime — `marked`, `dompurify` (2 packages; svelte compiles away). npm dev — vite, @sveltejs/vite-plugin-svelte, svelte, typescript, svelte-check (pinned lockfile, `npm ci`, `.npmrc ignore-scripts=true`, no postinstall execution). Explicitly rejected: OpenAI SDKs, web frameworks, SSE libs, chart libs (v1 sparkline is a 20-line SVG; `uplot` is the named F16 candidate), component libraries, Tailwind, router libs.
