...
Generated by `scripts/distill-save` (two passes: scan for the identifying strings and the story-script sizes, then stream-prune and scrub). It is the only `*.xml.gz` `.gitignore` lets into the repo.
...
**Scrubbed** (every attribute value and text node in the whole file, not just `<info>` — the surname also appears in log entries and in ships the player named):

| real value | replaced with |
| --- | --- |
| player name, and each of its word tokens | `Test Pilot` / `Test` / `Pilot` |
| game GUID (both cases) | `00000000-0000-4000-8000-000000000000` |
| seed | `1234567890` |
| game `code` | `0000000` |
| save name | `Distilled Fixture` |
| home directory path, OS username, OS full name | `redacted` |

Custom ship and station names the player typed are **kept** (`Mineral Hauler`, `Gas Hauler`, …) — they are gameplay labels, not identity, and they make the fixture worth having. `-scrub "a,b,c"` adds any term to both the scrub and the verification.

**Verification.** The tool re-reads the file it just wrote, scans every line for every term, and deletes the output if anything hits — so a dirty fixture cannot be sitting in the tree at commit time. It is re-runnable against the committed fixture at any time (the real values are deliberately not written down here — that would put in a public doc exactly what the fixture was scrubbed of; take them from the source save's `<info>`):

```
$ go run ./scripts/distill-save -verify internal/x4save/testdata/real/distilled-quicksave.xml.gz \
    -scrub "<player name>,<given name>,<surname>,<guid>,<seed>,<game code>,<save name>"
verify: distilled-quicksave.xml.gz against 10 term(s): […]
verify: 0 hits — clean
```

and independently, without trusting the tool at all:

```
$ zcat internal/x4save/testdata/real/distilled-quicksave.xml.gz \
    | grep -c -E '<given name>|<surname>|<guid prefix>|<seed>|<game code>|<home dir>'
0
```

Run on 2026-08-10 against the source quicksave's real values, both returned **0 hits** for all 10 terms.

`TestVerifyFixtureFindsAPlantedTerm` in `scripts/distill-save` guards the guard: it requires the scanner to *find* a term that is genuinely in the file, so "0 hits" cannot mean "not looking".
